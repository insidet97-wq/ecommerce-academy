...
| `/login` | Login (email/password OR Google OAuth) | No |
| `/signup` | Signup (email/password OR Google OAuth) → welcome email → redirect to module | No |
| `/auth/callback` | Post-Google-OAuth landing — sets up the profile, captures quiz/referral, sends welcome email, redirects to dashboard | No |
...
| `/about` | About page (team story, mission, what FSL is and isn't) | No |
| `/contact` | Contact form (name + email + subject + message → support@ via Resend, with honeypot + per-IP rate limit) | No |
...
| What | Detail |
|------|--------|
| **Sign in / Sign up with Google (OAuth)** | New auth path next to email/password. Supabase brokers the OAuth handshake — we don't write any token-handling code. **`components/GoogleSignInButton.tsx`** is a reusable button (white-on-gray, official Google G logo inlined as SVG, brand-guideline compliant) that calls `supabase.auth.signInWithOAuth({ provider: "google", redirectTo: /auth/callback })`. **`app/auth/callback/page.tsx`** is the landing page after Google bounces back: polls `getSession()` for ~5s until Supabase surfaces the session, then runs first-time-user setup if `user_profiles` has no row yet — pulls `first_name` from Google metadata (`given_name` → `full_name` split → `name` split → empty), persists any `quiz_results` from localStorage (so OAuth users still get a personalised roadmap), captures referral via `/api/referrals/track`, and fires `/api/send-welcome` (fire-and-forget, doesn't block the redirect). Then forwards to `/dashboard` (or to `ea_next` for deep-link redirects). On failure shows a clear error + Try Again link rather than spinning forever. **`/login` and `/signup`** both show the Google button above the email/password form with an "or" divider. Existing email/password flow untouched — both paths work in parallel. **Privacy policy** updated to list Google as a processor that receives auth events (email + name + profile picture) for users who pick that path. **Owner did the platform-side setup** (Google Cloud Console: OAuth consent screen published with non-sensitive scopes — no Google verification needed; OAuth Client ID with Supabase callback URL; Supabase Auth → Providers → Google enabled with Client ID + Secret pasted). No SQL changes |
| **About page + Contact form for AdSense resubmission credibility** | AdSense rejected the site with the generic "Programme policies" reason. Most likely cause: missing About + Contact pages (publisher-info requirements). Built both. **`/about`** is a substantial page (~700 words) with the team's real story — small team of engineers, struggled through ecommerce themselves, built the curriculum they wished existed. Sections: the problem we ran into, what we built, how we're different, who FSL is for / isn't for, why we stay anonymous. **`/contact`** is a feedback form (NOT an exposed email — owner's preference) with name + email + subject (dropdown of 6 categories) + message; client-side validation; honeypot field hidden offscreen for bot defense; new `POST /api/contact` route validates inputs, runs a per-IP rate limit (3/hour, in-memory Map), then forwards to support@firstsalelab.com via Resend with `replyTo` set to the sender so hitting Reply in our inbox goes back to them. Both pages added to `sitemap.ts` (priority 0.6 / 0.5) and `robots.ts` allow list. Footer in `app/page.tsx` updated to link both pages on logged-out and logged-in views. Once Vercel redeploys, owner clicks Resubmit on AdSense |
| **Hard-refresh access-drop bug fixed + 3 missing user_profiles columns added + $19/$49 payment flows fully verified** | Spotted during $19 Pro testing: paid user could pay, see Pro on dashboard, but on hard refresh the dashboard would show Free state. Root cause: dashboard's user_profiles fetch enumerated 9 specific columns and one (`streak_days`) didn't exist in production — the whole select returned 400 Bad Request, profile state defaulted to `is_pro=false`. The optimistic `?upgraded=pro` URL-param state was masking it on first load. **Fix in 2 parts:** (a) dashboard query switched from explicit column list to `select("*")` — now resilient to schema drift; one missing column no longer kills the whole fetch, just shows up as `undefined` in the destructure. (b) **3 missing columns added** to the master SQL migration block — `streak_days int DEFAULT 0`, `last_active date`, `start_module int DEFAULT 1`. Owner ran the migration in Supabase. With these in place, the streak counter (which had silently never worked since the streak feature was added) starts incrementing for the first time. **End-to-end payment flow now fully verified:** $19 Pro and $49 Scale Lab both checkout → webhook → DB update → optimistic UI → persists across hard refresh and logout/login → Billing button opens Stripe portal (now in a new tab — UX preference) → cancel-at-period-end keeps access during grace period → cancel-immediately fires `customer.subscription.deleted` → flags drop to false → dashboard reverts to Free. Welcome emails arrive correctly. Diagnostic console.warn that helped find this is now removed |
...
### 📜 Full SQL migrations (run all once in the Supabase SQL editor — `IF NOT EXISTS` makes them safe to re-run)
  ...
  -- 🔄 Streak / progress columns — discovered missing during $19 Pro test on
  -- 2026-04-30. Dashboard's profile fetch was 400'ing because streak_days
  -- didn't exist; the /api/cron/streak-save and /api/cron/digest jobs were
  -- silently failing on read for the same reason. Run these on any DB that
  -- pre-dates that fix.
  ALTER TABLE user_profiles ADD COLUMN IF NOT EXISTS streak_days  int  NOT NULL DEFAULT 0;
  ALTER TABLE user_profiles ADD COLUMN IF NOT EXISTS last_active  date;
  ALTER TABLE user_profiles ADD COLUMN IF NOT EXISTS start_module int  NOT NULL DEFAULT 1;
