...
| Date | What changed |
|------|-------------|
| 2026-05-01 | **Pre-launch security pass 3 — final hardening before opening the floodgates.** Third audit done now that Stripe is in live mode taking real money. Most "critical/high" agent findings turned out to be false positives once verified against the actual code: email-header injection via `replyTo` (regex `\s` already blocks `\n`/`\r`); `blog_posts` cascade-on-delete (table has no `user_id` FK so nothing to cascade); Niche Picker email-case rate-limit bypass (already lowercases email). Real findings shipped: (1) **Contact form** — explicit `\r\n` rejection on email (defense-in-depth even though regex covers it) + 50-char cap on `subject` field for future-proofing. (2) **Dashboard self-heal + OAuth callback** — `first_name` now `slice(0, 30)` on upsert to match the display-side cap; keeps DB and UI in sync. (3) **Password strength** — `/signup` and `/reset-password` now require minimum 8 chars + at least one letter + one number (was just `>= 6`). Supabase default is 6 — we override on the client because with Stripe live, weak passwords are a real account-takeover vector for paying users. **Items knowingly not addressed yet:** in-memory rate-limit on contact form (per-Vercel-instance, bounded by honeypot at our scale), no CSP header (inline styles + AdSense + GA4 + Stripe.js make a strict CSP painful — flagged for future work). **Launch readiness ~90/100.** Structural security has been hit by three independent audit passes; payment flow verified end-to-end with real card; OAuth resilience tested. The remaining gaps are polish, not blockers |
